...
export default function WelcomeModal({ companyName, token, onDismiss }: WelcomeModalProps) {
  ...
  return (
    ...
    >
      <div
        className="relative w-full max-w-md bg-[var(--card)] rounded-2xl shadow-2xl border border-[var(--border2)] p-8"
        style={{
      ...
      >
        ...
        <div className="flex justify-center gap-2 mb-6">
          <span
            className={`h-1.5 rounded-full transition-all ${step === 1 ? 'w-6 bg-[var(--green-fill)]' : 'w-1.5 bg-[var(--border)]'}`}
            aria-hidden
          />
          <span
            className={`h-1.5 rounded-full transition-all ${step === 2 ? 'w-6 bg-[var(--green-fill)]' : 'w-1.5 bg-[var(--border)]'}`}
            aria-hidden
          />
        </div>
        ...
        {step === 1 ? (
          <div className="text-center">
            ...
            <p className="text-base text-[var(--muted)] mb-6">We&apos;re glad to have you onboard!</p>
            <button
              type="button"
              onClick={() => setStep(2)}
          ...
          <div className="text-center">
            ...
            <h2 className="text-xl font-bold text-[var(--text)] mb-2">Let&apos;s get to know you</h2>
            ...
            <button
              ...
              disabled={submitting}
              className="w-full py-3 bg-[var(--green-fill)] text-[var(--on-green)] rounded-lg font-semibold hover:bg-[var(--green-dim)] transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
            >
